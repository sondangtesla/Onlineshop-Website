    ...
    <div class=" main-content-area">
            ...
            <ul class="products-cart">
                ...
                <li class="pr-cart-item">
                    ...
                    @if($item->model->sale_price > 0 && $sale->status == 1 && $sale->sale_date > Carbon::now())  
                        <div class="price-field product-price"><p class="regprice">@currency($item->model->sale_price)</p><del><p class="product-price">@currency($item->model->regular_price)</p></del></div>                                 
                    @else   
                        <div class="price-field product-price"><p class="price">@currency($item->model->regular_price)</p></div>
                    @endif
        ...
        <div class="summary">
            <div class="order-summary">
                <h4 class="title-box">Pengiriman</h4>
                <p class="summary-info"><span class="title">Biaya Pengiriman</span><b class="index">
                <select class="form-control" name="" style="width:500px; height:40px" wire:model="ongkir" required>
                        <option value="0">--PILIH PENGIRIMAN--</option>
                        
                        @foreach($result as $row)
                        <option style="height:40px"  value="{{$row['biaya']}}">
                        
                        <h6><strong>{{$row['description']}} | </strong></h6>
                        <h5><strong>@currency($row['biaya']) | </strong></h5>
                        <h6><strong> Perkiraan Sampai : {{$row['etd']}} hari</strong></h6>
                        </option>
                        @endforeach
                    </select>
                </b></p>
        ...
        <div class="summary">
            <div class="order-summary">
                ...
                <p class="summary-info"><span class="title">Subtotal</span><b class="index">@currency(Cart::instance('cart')->subtotal())</b></p>
                <p class="summary-info"><span class="title">Pengiriman</span>
                @if($ongkir)
                <b class="index">@currency($ongkir)</b>
                @else
                <b class="index" style="color: red;">Silakan pilih  pengiriman</b> 
                @endif</p>
                @if(Session::has('coupon'))
                <p class="summary-info"><span class="title">Discount ({{Session::get('coupon')['code']}})</span><b class="index">@currency($discount)</b></p>
                <p class="summary-info"><span class="title">Tax ({{config('cart.tax')}}%)</span><b class="index">@currency($taxAfterDiscount)</b></p>
                <p class="summary-info"><span class="title">Subtotal with Discount</span><b class="index">@currency($subtotalAfterDiscount)</b></p>
                <p class="summary-info total-info "><span class="title">Total</span><b class="index">@currency($totalAfterDiscount)</b></p>
                
                @else
                <p class="summary-info"><span class="title">Tax</span><b class="index">@currency(Cart::instance('cart')->tax())</b></p>
                
                
                <p class="summary-info total-info "><span class="title">Total</span><b class="index">@currency($totalAfterOngkir)</b></p>
                @endif
            </div>
                @if(!Session::has('coupon'))
                ...
                @else
                <br><br>
                <h4><b class="index" style="color: red;">Silakan pilih  pengiriman</b>  </h4>
